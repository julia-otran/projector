...
__UINT64 randomseed;                     /* Current random number seed. */
...
/*  triangles are four-byte-aligned and that the `__UINT64' type is     */
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((__UINT64) (ptr) & (__UINT64) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((__UINT64) (ptr) ^ (__UINT64) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((__UINT64) (otri).tri | (__UINT64) (otri).orient)
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
                    ((__UINT64) (otri).tri[6] | (__UINT64) 2l)
...
#define infected(otri)                                                        \
  (((__UINT64) (otri).tri[6] & (__UINT64) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((__UINT64) (sptr) & (__UINT64) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((__UINT64) (sptr) & ~ (__UINT64) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((__UINT64) (osub).ss | (__UINT64) (osub).ssorient)
...

{
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (__UINT64) printvertex,
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (__UINT64) printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
    ...
    printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
           t->orient + 3, (__UINT64) printvertex,
           printvertex[0], printvertex[1]);
...

{
  ...
  printf("subsegment x%lx with orientation %d and mark %d:\n",
         (__UINT64) s->ss, s->ssorient, mark(*s));
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           2 + s->ssorient, (__UINT64) printvertex,
           printvertex[0], printvertex[1]);
  sdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Dest  [%d] = NULL\n", 3 - s->ssorient);
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           3 - s->ssorient, (__UINT64) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
           4 + s->ssorient, (__UINT64) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
           5 - s->ssorient, (__UINT64) printvertex,
           printvertex[0], printvertex[1]);
}
...

{
  ...
  alignptr = (__UINT64) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
    (alignptr + (__UINT64) pool->alignbytes -
     (alignptr % (__UINT64) pool->alignbytes));
...

{
  ...
  alignptr = (__UINT64) (pool->pathblock + 1);
...

{
  ...
  alignptr = (__UINT64) m->dummytribase;
  m->dummytri = (triangle *)
    (alignptr + (__UINT64) m->triangles.alignbytes -
     (alignptr % (__UINT64) m->triangles.alignbytes));
  ...
  if (b->usesegments) {
    ...
    alignptr = (__UINT64) m->dummysubbase;
    m->dummysub = (subseg *)
      (alignptr + (__UINT64) m->subsegs.alignbytes -
       (alignptr % (__UINT64) m->subsegs.alignbytes));
...

{
  ...
  alignptr = (__UINT64) (getblock + 1);
  foundvertex = (char *) (alignptr + (__UINT64) m->vertices.alignbytes -
                          (alignptr % (__UINT64) m->vertices.alignbytes));
...
#ifdef ANSI_DECLARATORS
__UINT64 randomnation(unsigned int choices)
#else /* not ANSI_DECLARATORS */
__UINT64 randomnation(choices)
...

{
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (__UINT64) (sampleblock + 1);
    firsttri = (char *) (alignptr +
                         (__UINT64) m->triangles.alignbytes -
                         (alignptr %
                          (__UINT64) m->triangles.alignbytes));
